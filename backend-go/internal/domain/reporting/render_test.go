...
func sampleReport() reporting.Report {
	...
	return reporting.Report{
		...
		ByHour:     []reporting.HourLine{{Hour: 9, Revenue: 54450, Orders: 1}},
		ComputedAt: &computed,
	}
}
...
func TestACSVExportIsExcelReadableAndCannotRunAFormula(t *testing.T) {
	...
	require.True(t, text("Pendapatan", "77450"), "money as plain integers")
	...
	require.True(t, text("09:00", "54450", "1"))
...
func TestAnXLSXExportIsAWorkbookWithNumbersAsNumbers(t *testing.T) {
	...
	for _, name := range []string{"[Content_Types].xml", "_rels/.rels", "xl/workbook.xml", "xl/_rels/workbook.xml.rels", "xl/styles.xml", "xl/worksheets/sheet1.xml", "xl/worksheets/sheet9.xml"} {
		require.Contains(t, parts, name)
	}
	require.Equal(t, 9, strings.Count(parts["xl/workbook.xml"], "<sheet "))
	require.Contains(t, parts["xl/workbook.xml"], `name="Audit diskon &amp; void"`)

	summary := parts["xl/worksheets/sheet1.xml"]
	require.Contains(t, summary, `<c r="B5" s="2"><v>77450</v></c>`, "revenue is a number cell")
	require.Contains(t, summary, `<v>60.00</v>`)

	categories := parts["xl/worksheets/sheet4.xml"]
