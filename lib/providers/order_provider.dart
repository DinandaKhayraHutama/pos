...
Future<Order> placeOrderFromCart({
  ...
  final serviceChargeRate =
      settings.serviceChargeEnabled ? settings.serviceChargeRate : 0.0;
  ...
  final order = await OrderRepository.instance.create(
    ...
    posName: settings.posRegisterName.isEmpty
        ? null
        : settings.posRegisterName,
  ...
  // If dine-in and a table was assigned, mark table as occupied for the demo.
  if (cart.type == OrderType.dineIn && table != null) {
    await TableRepository.instance.setStatus(table.id, TableStatus.occupied);
...
class TablesNotifier extends AutoDisposeAsyncNotifier<List<RestaurantTable>> {
  ...
  Future<void> setStatus(String id, TableStatus status) async {
    await TableRepository.instance.setStatus(id, status);
    await refresh();
  }
}
...
final activeTablesProvider =
    AsyncNotifierProvider.autoDispose<ActiveTablesNotifier, List<RestaurantTable>>(
