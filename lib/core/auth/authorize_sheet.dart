...
Future<Employee?> requestAuthorization(
  ...
  return showGlassSheet<Employee>(
    ...
    builder: (ctx) => _PinPromptSheet(
      ...
      // No account list here, unlike the handover. This prompt is held out to
      // someone who is not standing at the till, and naming the two people who
      // can approve turns "get a manager" into a shortlist for anyone who
      // wants to guess a PIN. The handover has no such problem: whoever is
      // taking over already knows who they are.
      pickAccount: false,
    ),
  );
}
...
class _PinPromptSheetState extends State<_PinPromptSheet> {
  ...
  Future<void> _onDigit(String digit) async {
    ...
    setState(() => _busy = true);
    // Three shapes, one keypad. [fixedId] wins when set — the re-confirmation
    // case, where only that one identity may ever pass. Otherwise: scoped to
    // the chosen account when there was one, so tapping a name and typing
    // someone else's PIN hands the till to the person on the tile you did not
    // touch — which is exactly the confusion the picker removes.
    final employee = fixedId != null
        ? await EmployeeRepository.instance.verify(id: fixedId, pin: typed)
        : account == null
        ? await EmployeeRepository.instance.byPin(typed)
