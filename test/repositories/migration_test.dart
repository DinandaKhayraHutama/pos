...
void main() {
  ...
  group('DB migration chain', () {
    test('v1 → v4: v2 adds product columns, v4 adds category icon_key', () async {
      ...
      final v1ProductCols = _columns(await v1Db.rawQuery(
        'PRAGMA table_info(products)',
      ));
      expect(v1ProductCols.contains('image_url'), isFalse,
          reason: 'image_url is a v2 column, must not exist at v1');
      expect(v1ProductCols.contains('icon_key'), isFalse,
          reason: 'products.icon_key is a v2 column, must not exist at v1');

      final v1CategoryCols = _columns(await v1Db.rawQuery(
        'PRAGMA table_info(categories)',
      ));
      expect(v1CategoryCols.contains('icon_key'), isFalse,
          reason: 'categories.icon_key is a v4 column, must not exist at v1');

      // user_version is 1.
      final v1Version =
          (await v1Db.rawQuery('PRAGMA user_version')).first['user_version'];
      expect(v1Version, 1);
      ...
      final v4ProductCols = _columns(await v4Db.rawQuery(
        'PRAGMA table_info(products)',
      ));
      expect(v4ProductCols.contains('image_url'), isTrue,
          reason: 'v2 must have added products.image_url');
      expect(v4ProductCols.contains('icon_key'), isTrue,
          reason: 'v2 must have added products.icon_key');

      final v4CategoryCols = _columns(await v4Db.rawQuery(
        'PRAGMA table_info(categories)',
      ));
      expect(v4CategoryCols.contains('icon_key'), isTrue,
          reason: 'v4 must have added categories.icon_key');

      // user_version bumped to 4.
      final v4Version =
          (await v4Db.rawQuery('PRAGMA user_version')).first['user_version'];
      ...
      expect(categories.first['icon_key'], 'set_meal',
    ...
    test('v2 → v4: v3 nulls the dead seed image URLs', () async {
      ...
      final deadAfter = (await v4Db.query(
        ...
        where: "id = 'p_dead'",
          ...
          .first;
      expect(deadAfter['image_url'], isNull,
      ...
      final liveAfter = (await v4Db.query(
        ...
        where: "id = 'p_live'",
          ...
          .first;
      expect(liveAfter['image_url'], _liveImageUrl,
    ...
    test('v11 renames the seeded staff everywhere their name was copied',
        () async {
      ...
      expect(order['authorized_by'], 'Siwi Wiyono Raharjo',
          reason: 'the approver is recorded by name; it has to move too');

      expect((await db.query('shifts')).first['employee_name'],
          'Siwi Wiyono Raharjo');
      expect((await db.query('stock_movements')).first['employee_name'],
          'Siwi Wiyono Raharjo');
      ...
      expect(renamed['name'], 'Nama Pilihan Sendiri',
    ...
    test('v1 → v16: the whole chain lands on tills', () async {
      ...
      final shiftCols = _columns(await db.rawQuery('PRAGMA table_info(shifts)'));
      expect(shiftCols, containsAll(<String>['pos_id', 'pos_name', 'outlet_id',
          'outlet_name', 'closed_by_id', 'closed_by_name']));

      final orderCols = _columns(await db.rawQuery('PRAGMA table_info(orders)'));
      expect(orderCols,
    ...
    test('v15 → v16 leaves real sales without a till rather than guessing',
        () async {
      ...
      expect(shift['closed_at'], isNull,
          reason: 'a live drawer must not be closed against a count nobody made');
    ...
    test('one till cannot hold two open sessions', () async {
      ...
      expect(
        Sqflite.firstIntValue(
          await db.rawQuery('SELECT COUNT(*) FROM shifts WHERE closed_at IS NULL'),
        ),
        1,
      );
    });
    ...
    test(
      'v1 → v17: modifier tables land, and seeding skips products that are '
      'not there — mirroring seedVariants',
      () async {
        ...
        expect(orderItemCols, containsAll(<String>['category_id', 'category_name']));
        ...
        expect(await db.query('product_modifier_groups'), isEmpty,
            reason:
    ...
    test(
      'v16 → v17 backfills category_id/category_name on order lines whose '
      'product still exists, and leaves an orphaned line NULL',
      () async {
        ...
        expect(orphaned['category_id'], isNull,
            reason:
    ...
    test(
      ...
      () async {
        ...
        expect(groupOptions, isNotEmpty,
            reason: 'the seeded groups all have options; a group with none '
        ...
        final scopedIds = scopeRows.map((r) => r['option_id'] as String).toSet();

        expect(scopedIds, activeIds,
            reason: 'every currently-active option in the attached group '
                'must be backfilled, so an existing attachment keeps '
                'behaving exactly as it did before per-product scoping '
                'existed');
        expect(scopedIds.contains(deactivatedId), isFalse,
            reason: 'an inactive option was never actually offered, so '
    ...
    test(
      'v18 → v19 adds pb1_rate / service_charge_rate / '
      'service_charge_amount, defaulting existing rows to NULL rate / 0 '
      'amount rather than backfilling a guess',
      () async {
        ...
        final existingOrderId =
            (await v18Db.query('orders', limit: 1)).first['id'];
        await v18Db.close();
        ...
        final orderCols = _columns(await db.rawQuery(
        ...
        expect(row['pb1_rate'], isNull,
            reason: 'the PB1 rate in effect for a pre-v19 order is '
                'genuinely unrecoverable if the store rate ever changed — '
                'must stay NULL, not guess');
        expect(row['service_charge_rate'], isNull);
        expect(row['service_charge_amount'], 0,
            reason: 'the feature did not exist yet, so 0 is a fact for a '
    ...
    test(
      ...
      () async {
        ...
        final existingTableId =
            (await v20Db.query('tables', limit: 1)).first['id'];
        await v20Db.close();
        ...
        expect(
          row['active'],
          1,
          reason: 'a table that already existed was already visible on the '
