...
class RestaurantTable {
  ...
  /// screen stores whatever the admin typed and shows it back verbatim. On an
  /// activated till it is the server's `area`.
  final String floor;
  final int sortOrder;
  ...
  /// The server's word that two tills changed this table at almost the same
  /// time (Fase 6). Shown to staff, never resolved silently; the next change
  /// made against the current status clears it. Always false in the demo.
  ...
  factory RestaurantTable.fromMap(Map<String, dynamic> m) => RestaurantTable(
    ...
    contested: ((m['contested'] as int?) ?? 0) == 1,
  );

  /// The demo editor's row. The server columns are not in it: they are
  /// written only by a pull.
