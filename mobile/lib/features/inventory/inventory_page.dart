...
String stockReasonLabel(AppLocalizations l10n, StockReason reason) =>
    switch (reason) {
      ...
      StockReason.count => l10n.stockReasonCount,
      StockReason.transferIn => l10n.stockReasonTransferIn,
      StockReason.transferOut => l10n.stockReasonTransferOut,
    };
...
class _AdjustSheetState extends ConsumerState<_AdjustSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final isCount = _reason == StockReason.count;
    // A count replaces the balance; anything else moves it. Floored at zero in
    // the demo only — an activated till shows the shortfall it will record.
    final next = isCount
        ? qty
        : TillBinding.current != null
        ? current + (_isIncrease ? qty : -qty)
        : (current + (_isIncrease ? qty : -qty)).clamp(0, 1 << 31);
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              for (final r in [...StockReasonX.manual, StockReason.count])
                ChoiceChip(
                  selected: _reason == r,
          ...
          const SizedBox(height: AppDimensions.space12),
          GlassTextField(controller: _noteCtrl, label: l10n.inventoryNoteHint),
          const SizedBox(height: AppDimensions.space16),
          FilledButton.icon(
            // Zero is a legitimate count (an empty shelf); an empty field is
            // not a count at all.
            onPressed:
                _busy || (isCount ? _qtyCtrl.text.trim().isEmpty : qty <= 0)
                ? null
                : _save,
            icon: Icon(
              isCount
  ...
  Future<void> _save() async {
    ...
    final qty = int.tryParse(_qtyCtrl.text) ?? 0;
    final isCount = _reason == StockReason.count;
    if (isCount ? _qtyCtrl.text.trim().isEmpty : qty <= 0) return;
    setState(() => _busy = true);
    ...
    try {
      ...
      final balance = isCount
          ...
          : await StockRepository.instance.adjust(
              outletId: outletId,
              productId: widget.product.id,
