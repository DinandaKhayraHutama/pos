...
class _PosSessionOpenCardState extends ConsumerState<PosSessionOpenCard> {
  ...
  Future<void> _open() async {
    ...
    try {
      ...
      showAppSnackBar(context, context.l10n.tillBindingMismatch, error: true);
      ...
      child: Column(
        ...
        children: [
          ...
          totals.when(
            ...
            data: (t) {
              final sums =
                  t ?? const ShiftTotals(cash: 0, nonCash: 0, orderCount: 0);
              return Column(
                children: [
...
class _CloseShiftSheetState extends ConsumerState<_CloseShiftSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final expected = widget.shift.openingCash + (totals?.cash ?? 0);
